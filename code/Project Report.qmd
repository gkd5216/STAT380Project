...
We then examined both the train and test sets to see where the missing values were, and if we could eliminate any. Below is the output for the train and test datasets respectively.
...
Only the drug test dataset outputted with a missing value being present, so we figured out which column contains it. Then, we examined which columns in the test dataset had missing values using colSums.

```{r echo=FALSE, message=FALSE}
colSums(is.na(drug_test_tsv))
```

We figured out that the commentsReview column contains 1 missing value, but it is not specifically an N/A column. It states "N/A currently," so it did not seem necessary to take care of this. Beyond handling the missing values, preprocessing steps were tailored based on the needs of the various machine learning models.
...
#| fig-height: 5
#| fig-width: 7
#| fig-alt: "Table showing Exploratory Data Analysis"
#| aria-describedby: namesPlotLD
#| lst-label: lst-figure2
#| lst-cap: "Code Chunk for Making Figure 1"
...
**K-means Clustering** is an unsupervised learning model. It is used to discover hidden or underlying patterns and groupings in the data without using labels. It aims to identify the clusters of patient experiences with specific drugs along with uncovering underlying patterns in patient satisfaction. We also aim to find groups of reviews with similar properties.
...
```{r echo=FALSE, message=FALSE}
#| label: LRConfusionMatrix
#| fig-cap: "Logistic Regression Confusion Matrix"
#| fig-pos: H
#| fig-height: 5
#| fig-width: 7
#| fig-alt: "Table showing Logistic Regression Confusion Matrix"
#| aria-describedby: namesPlotLD
#| lst-label: lst-figure2
#| lst-cap: "Code Chunk for Making Figure 1"
...
```{r echo=FALSE, message=FALSE}
#| label: NNConfusionMatrix
#| fig-cap: "Neural Networks Confusion Matrix"
#| fig-pos: H
#| fig-height: 5
#| fig-width: 7
#| fig-alt: "Table showing Neural Networks Confusion Matrix"
#| aria-describedby: namesPlotLD
#| lst-label: lst-figure2
#| lst-cap: "Code Chunk for Making Figure 1"
...
# Groups by rating, drug names, conditions, and effectiveness
train_data <- drug_train_tsv %>%
...
```{r echo=FALSE, message=FALSE}
#| label: KNNConfusionMatrix
#| fig-cap: "KNN Confusion Matrix"
#| fig-pos: H
#| fig-height: 5
#| fig-width: 7
#| fig-alt: "Table showing KNN Confusion Matrix"
#| aria-describedby: namesPlotLD
#| lst-label: lst-figure2
#| lst-cap: "Code Chunk for Making Figure 1"
...
# Recode less common ones as titled "Other"
...
```{r echo=FALSE, message=FALSE}
#| label: KMeansConfusionMatrix
#| fig-cap: "K-Means Confusion Matrix"
#| fig-pos: H
#| fig-height: 5
#| fig-width: 7
#| fig-alt: "Table showing K-Means Confusion Matrix"
#| aria-describedby: namesPlotLD
#| lst-label: lst-figure2
#| lst-cap: "Code Chunk for Making Figure 1"
...
# Groups by ratings, top drugs, and top conditions
...
fviz_cluster(list(data = kmeans_input, 
             ...
             geom = "point",
             ellipse.type = "norm",
             repel = TRUE, #Avoid overclustering
             palette = "lancet", #Soft pastel 
...
The K-means Clustering model displays the discovery of patient review groups. It includes 5 clusters that are shown with normal ellipses.

The first two principal components captured only 6.1% of the variance which is quite low. This suggests that the drug reviews dataset has a complex structure that cannot be fully visualized in two dimensions; however, the PCA plot outputted some useful information. Cluster 4 had the widest spread which represents a greater diversity in the patient experiences. Cluster 2 had a tightly grouped cluster which represents reviews with similar characteristics. Clusters 1 and 5 had an overlapping cluster which represents shared features. Lastly, Cluster 3 showed a compact and separate cluster which possibly represents a consistent trend of patient satisfaction, either high or low.

The K-means Clustering model displayed that patient satisfaction is influenced by the combinations of the medication and the condition that is being treated, and not just by the ratings. Some of the clusters likely would correspond to the reviews of popularly prescribed drugs with a predictable effectiveness, such as Pain Killers or Tylenol. The trends shown suggest that an unsupervised learning model (K-means) can uncover meaningful subgroups in healthcare and medicine. This could potentially uplift future segmentation or a follow-up analysis.

# Discussion

The results of our analysis suggest that different machine learning models exhibit varying levels of effectiveness when applied to patient drug reviews, particularly in predicting overall satisfaction or sentiment. The **logistic regression** model stood out as one of the most reliable methods, achieving a high accuracy of 82.85% in predicting whether a review was positive or negative. It performed especially well in identifying positive reviews, evidenced by a specificity of 91.79%. This indicates the model's strong ability to capture the cues that signify satisfaction. However, its sensitivity for negative reviews was noticeably lower (66.46%), suggesting that the model struggled to distinguish some dissatisfied users, likely due to class imbalance, where the majority of reviews are positive.

The **neural network** model provided a more detailed approach by attempting to predict exact numerical ratings on a 1–10 scale. While it demonstrated some strength in identifying the most common rating (10), its overall accuracy of 39.8% reveals significant limitations. The model performed poorly on mid-range and less common ratings like 2, 5, and 9. This issue is largely attributable to the imbalanced distribution of scores and the lack of informative input features. The model relied solely on structured categorical data (e.g., drug name, condition, and effectiveness), so it was likely unable to detect subtle differences in patient sentiment. Additionally, the neural network architecture—featuring only a single hidden layer with 5 neurons—may have lacked the ability to capture complex nonlinear relationships in the data.

On the other hand, the **K-Nearest Neighbors (KNN)** model showed modest predictive ability with an overall accuracy of 65.2%, just slightly better than the No Information Rate of 64.7%. The model's low Kappa value of 0.0556 indicates minimal agreement between predicted and actual sentiments, revealing the model’s overreliance on majority class predictions. The confusion matrix further showed an imbalanced performance, with the model correctly identifying a large number of negative reviews but failing to capture most of the positive ones. This result illustrates a common limitation of KNN in classification problems: while easy to implement, it can “memorize” the training data rather than learn general patterns, especially when the feature space is limited and imbalanced.

Lastly, **K-means clustering**, as an unsupervised method, offered another perspective by uncovering hidden groupings in the dataset. While it does not predict ratings or sentiments directly, it helped identify clusters of reviews with similar properties. Clusters 1 and 5 emerged as the most distinct and informative, strongly associated with higher patient ratings. On the other hand, clusters 2, 3, and 4 showed more mixed or lower ratings, suggesting potential for identifying patient subgroups with dissatisfaction profiles. Though clustering lacks the interpretability of supervised models, its strength lies in pattern discovery and could inform future strategies for targeted analysis.

A significant challenge in this project was being able to manage the high dimensionality and complexity of the dataset. This specifically relates to modeling drug names and conditions which involved many unique values. We needed to perform careful feature engineering and dimensional reduction to ensure that the models were stable and interpretable.

# Conclusion

In summary, our findings show that machine learning can be effectively applied to drug review data to predict patient satisfaction. However, the success of prediction varies significantly depending on the chosen model and the nature of the outcome. **Logistic regression** proved to be the most balanced and interpretable model for binary sentiment classification, making it a practical tool for summarizing patient opinions at scale. **Neural networks**, while promising for detailed predictions, require more nuanced data and better handling of class imbalance to reach their full potential. **KNN** showed limited generalization capability, and **K-means clustering** was most useful in uncovering dominant patterns in the data.

Future work should consider incorporating natural language processing (NLP) techniques to extract sentiment-rich features —particularly benefits, side effects, and comments—which would likely improve overall model performance. Addressing class imbalance through resampling could also enhance the model’s ability to identify less frequent sentiments or ratings. Overall, this project emphasizes the value of machine learning in improving our understanding of real-world patient experiences and supports its application in clinical decision-making and public health.
