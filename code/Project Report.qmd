...
# Collapse the less common values
...
set.seed(42)
train_ind <- createDataPartition(drug_train$sentiment, p = 0.8, list = FALSE)
train_set <- drug_train[train_ind, ]
test_set <- drug_train[-train_ind, ]

# Encoding
train_encode <- model.matrix(~ top_drugs + top_conditions - 1, data = train_set)
test_encode <- model.matrix(~ top_drugs + top_conditions - 1, data = test_set)

train_labels <- train_set$sentiment
test_labels <- test_set$sentiment

knn_pred <- knn(train = train_encode, 
                test = test_encode, 
                cl = train_labels, k = 5)
...
The dataset used was the **drugLibTrain_raw.tsv** file from which we wrangled the data and selected features for the K-means model. Missing values were filtered out of the raw train data. We grouped less frequent occurrences into a category titled "Other" utilizing the top drugs and conditions defined prior.
...
The overall accuracy of the model is 0.6522 which means that it correctly classified about 65.22% of the reviews in the test set. There is a No Information Rate (NIR) of 0.6473 which represents the accuracy achieved predicting the majority class. On the other hand, the Kappa is very low at 0.0556 which represents only slight agreement between the KNN model's predictions and the actual sentiment.

**True Negatives (TN)** = 129; 129 reviews were predicted negative and actually negative

**False Positives (FP)** = 5; 5 reviews were predicted positive but actually negative

**False Negatives (FN)** = 67; 67 reviews were predicted negative but actually positive

**True Positives (TP)** = 6; 6 reviews were predicted positive and actually positive

**Total Observations** = 129 + 5 + 67 + 6 = 207 which is the size of the test set.
